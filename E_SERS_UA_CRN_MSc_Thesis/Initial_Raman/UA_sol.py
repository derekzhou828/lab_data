...
df = pd.read_csv('../Initial_Raman/UA_200uM_2.txt',
                 header=None, names=['wave', 'intst'], skiprows=range(14), sep='\t')
df = df[(df['wave'] > 500) & (df['wave'] < 1800)]
df['intst'] = (df['intst'] - ALSBaselineCorrection(df['intst'], p=0.001)) / (7 * 22.5)

df.to_csv('../Initial_Raman/UA_sol.csv', index=False)
...
plt.plot(wave, intst)
plt.ylim(-2, 20)
ax.set_title('Raman Spectrum for Uric Acid Solution', fontsize=14, pad=12)
...
plt.savefig('../Initial_Raman/UA_sol.png')
plt.show()
