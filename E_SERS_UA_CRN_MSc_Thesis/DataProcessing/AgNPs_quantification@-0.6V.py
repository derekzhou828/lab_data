...
ax.set_title('UA + 0.1M NaF + 10uM R6G for AgNPs-APTES-FTO: -0.6V', pad=10)
ax.set_title('Linear relationship: \n\ny = ' + str(format(slope, '.5f')) + 'x + '
             + str(format(intercept, '.5f')) + '\nR$^{2}$ = ' + str(format(r_value, '.5f')),
             loc='left', y=0.75, x=0.05, color='blue')
...
x_s = np.arange(1, 16)
plt.plot(x_s, linear_model_fn(x_s), color='blue', linestyle='--')
