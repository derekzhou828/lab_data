...
df_plot = df_plot[df_plot.Potential == 0].iloc[:, 4:].T.rename(columns=df_plot.UA)
...
ax.set_title('UA + 0.1M NaF + 10uM R6G for AgNPs-APTES-FTO at 0V', pad=10)
ax.set_xlabel('Raman Shift ($\mathregular{cm^{-1}}$)')
ax.set_ylabel('Intensity (counts $\mathregular{s^{-1}mW^{-1}}$)')

plt.savefig('../Data_Fig/UA_quantification_3.png')
plt.show()

df_linear = pd.read_csv('../Data_csv/UA_quantification_average.csv')
df_linear = df_linear[df_linear.Potential == 0].reset_index(drop=True)
...
df_all = df_all[df_all.Potential == 0].reset_index(drop=True)
...
ax = plt.subplot()
ax.set_title('UA + 0.1M NaF + 10uM R6G for AgNPs-APTES-FTO at 0V', fontsize=10)
...
plt.savefig('../Data_Fig/UA_quantification_4.png')
plt.show()
