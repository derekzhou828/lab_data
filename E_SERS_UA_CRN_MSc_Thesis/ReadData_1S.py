...
def ReadData(path_gamry='', path_raman='', duration=100):
    ...
    for file in files_raman:
        ...
        df['intst'] = (df['intst'] - ALSBaselineCorrection(df['intst'])) / (1*22.5)
        ...
        for i in range(len(time_line)):
            if (time_stamp > time_line[i]) & ((time_stamp + 10) <= time_line[i] + duration):
